...
class LogFile:
    ...
    def __init__(self, filename, delim, header, rows, time_attr, trace_attr, activity_attr = None, values = None, integer_input = False, convert = True, k = 1, dtype=None, role_type=False):
        ...
        if filename is not None:
            ...
            if role_type:
                print("types: ", self.data.dtypes)
                self.data['role'] = self.data['role'].astype("string")
                print("types after: ", self.data.dtypes)
            # Determine types for all columns - numerical or categorical
            for col_type in self.data.dtypes.iteritems():
                if col_type[1] == 'float64': #or col_type[1] == 'int64':
    ...
    def create_k_context_trace(self, trace):
        ...
        contextdata = pd.concat([contextdata, joined_trace])
